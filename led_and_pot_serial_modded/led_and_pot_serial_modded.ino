        ...
        if(Serial.available())
        {
            ...
            if(count >= packetSize)
            {
                ...
                if(validatePacket(packetSize, buffer))
                {
                    ...
                    Serial.print("\n");
                    Serial.print("New");
                    Serial.print(prev_potValue);
                      
                    if(potValue != prev_potValue)
                    {
                      buffer[0] = 0xAA;
                      buffer[1] = 0x05;
                      buffer[2] = 0x50;
                      ...
                      if(thing[0] == '1' && thing[1] == '1')
                      {
                        buffer[3] =0x03;  
                      }
                      else if(thing[0] == '1' && thing[1] == '0')
                      {
                        buffer[3] = 0x02; 
                       }
                       else if(thing[0] == '0' && thing[1] == '0')
                      {
                        buffer[3] = 0x00; 
                      }
                       else
                      {
                       ...
                       buffer[4]=(byte)strtol(thing+2,0,2)/4;
                         buffer[5] = buffer[0] ^buffer[1] ^buffer[2] ^buffer[3] ^ buffer[4];
                      sendPacket(6 - PACKET_OVERHEAD_BYTES, buffer + 2);
